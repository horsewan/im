...
public class ServerEventListenerImpl implements ServerEventListener
{
	...
	public boolean onTransBuffer_C2C_RealTimeSendFaild_CallBack(Protocal p)
	{
		...
		//System.out.println("【DEBUG_回调通知】[typeu="+typeu+"]客户端"+from_user_id+"发给客户端"+userId+"的消息：str="+dataContent
		//		+"，因实时发送没有成功，需要上层应用作离线处理哦，否则此消息将被丢弃."+"====="+fingerPrint);

		MongoUtil.insertOffLineMsg(userId,from_user_id,String.valueOf(typeu),fingerPrint,dataContent);
